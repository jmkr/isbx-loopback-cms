...
.directive('modelFieldImageEdit', function($compile, $document, GeneralModelService, ImageService, SessionService, $timeout, $translate) {
  ...
  return {
    restrict: 'E',
    require: '^form',
    template: '<div class="image-container" ng-class="{ \'display-only\' : displayOnly }" style="background: no-repeat center center url(\'{{ displayOnly ? imageUrl : thumbnailUrl }}\'); background-size: contain;" ng-click="!displayOnly && imageClick()"></div> \
